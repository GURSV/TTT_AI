...
function _HandlePlayerClick(evt) {
  ...
  if (isBlank && _GAMESTATE.active && _GAMESTATE.turn == "player") {
    evt.target.src = "cross.png";
    _CheckGameOver();
    _AISelectMove();
  }
}
...
function _CheckGameOver() {
  ...
  if (winner == BOARD_STATE.ai) {
    desc = "You lose! (Reload)";
  } else if (winner == BOARD_STATE.player) {
    desc = "You win! (Reload)";
  } else {
    desc = "Tie game, try again. (Reload)";
  }
...
function _GetBoardStates() {
  ...
  for (let x = 0; x < 3; x++) {
    ...
    for (let y = 0; y < 3; y++) {
      ...
      if (node.src.includes("cross.png")) {
        row.push(BOARD_STATE.player);
      } else if (node.src.includes("round.png")) {
...
function _AISelectMove(blinks) {
  ...
  if (choice != null) {
    const [x, y] = choice;
    document.getElementById(x + "." + y).src = "round.png";
  }
  ...
  for (let x = 0; x < 3; x++) {
    for (let y = 0; y < 3; y++) {
      if (boardStates[x][y] == BOARD_STATE.blank) {
        ...
        const [newMoveScore, _] = _Minimax(newBoardStates, BOARD_STATE.player);
  ...
  for (let x = 0; x < 3; x++) {
    for (let y = 0; y < 3; y++) {
      if (boardStates[x][y] == BOARD_STATE.blank) {
        ...
        const [newMoveScore, _] = _Minimax(newBoardStates, BOARD_STATE.ai);
